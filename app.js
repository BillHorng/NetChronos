...
function draw() {
  ...
  const points=data.map((s,i)=>({x:pad.left+(data.length===1?gw:i*gw/(data.length-1)),y:s.ok?pad.top+gh*(1-s.value/max):pad.top+4,ok:s.ok,value:s.value,time:s.time})); lastPoints=points;
  ctx.beginPath();points.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.lineTo(points.at(-1).x,pad.top+gh);ctx.lineTo(points[0].x,pad.top+gh);ctx.closePath();const fill=ctx.createLinearGradient(0,pad.top,0,pad.top+gh);fill.addColorStop(0,'rgba(79,169,207,.25)');fill.addColorStop(1,'rgba(79,169,207,0)');ctx.fillStyle=fill;ctx.fill();
  ctx.beginPath();points.forEach((p,i)=>i?ctx.lineTo(p.x,p.y):ctx.moveTo(p.x,p.y));ctx.strokeStyle='#5ed1d2';ctx.lineWidth=2;ctx.stroke();points.forEach(p=>{ctx.beginPath();ctx.arc(p.x,p.y,p.ok?3:4,0,Math.PI*2);ctx.fillStyle=!p.ok?'#ef6565':p.value>150?'#f4aa42':'#5ed1d2';ctx.fill();});
...
async function probe() {
  ...
  try {
    ...
    clearTimeout(timeout); samples = samples.slice(-240); updateStats();
