...
- 目前版本：`V1.0.5`
- 目前線上程式基準：`65b79e8`（修正低高度視窗監測按鈕被裁切）
- 最後交接更新：2026-09-09
...
- V1.0.5 修正較矮桌面視窗中開始／暫停按鈕被裁切：測試網站與連線識別改為同列，操作按鈕保留在面板底部可見區域。
- 桌面內容高度不超過 700px 時啟用低高度模式：縮小設定間距，A–D 評級改為四欄短標籤，優先保留開始／暫停操作。
...
- 維護提交 `65b79e8` 已推送至 `main` 並發布至 GitHub Pages。
...
- 修正新增連線識別後右側設定面板高度增加、導致開始／暫停按鈕在部分視窗被裁掉的布局回歸。
...
- 維護提交 `65b79e8` 已另以 1366 × 650、瀏覽器 100% 比例完成壓力驗證：低高度模式會壓縮設定間距並將 A–D 評級改為四欄短標籤，開始／暫停監測按鈕仍完整可見；未選擇網站時開始按鈕維持停用防呆。
